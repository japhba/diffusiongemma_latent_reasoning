...
_**Commitment order follows the task's logical direction — with a causal default.** Center of mass of the canvas positions committed by step $t$ (0 = left, 1 = right of the content span; 0.5 = fully committed) over diffusion progress $t/T$ (faint = single rollouts, bold = mean; frozen tail cropped; default sampler, argmax lock-in as commitment proxy; $\rho_{\mathrm{chain}}$ = median Spearman of final-commit step vs position). Dashed: matched left-to-right filler — a strictly causal fill sits on it. Left: GPQA (logically left-to-right) tracks the filler. Middle: poem writing (direction-indifferent) commits faster and less orderly, but still left-leaning — the inherited causal default. Right: the end-anchored `reverse_chain` (logically right-to-left, depths 4–5): solving runs (green) start at the anchored end and walk backward (digits-only $\rho_{\mathrm{chain}} = -0.89..-0.97$; panel $\rho$ is canvas-level), failing runs fill forward._
