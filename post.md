...
This simple response behavior begs the question whether it is possible to perturb $n>1$ source letters _simultaneously_ (again, keeping them subleading) and see whether the corresponding target images respond. This would be interesting, since it would point to the model's capacity to carry out simultaneous computation, a key advantage that latent reasoning models have on paper.
...
![Parallel injection fraction](figs/fig2c_parallel_frac.png)
