...
_**Commitment is left-to-right on benchmarks; anchored tasks deviate.** Center of mass of the canvas positions committed by step $t$ (0 = left, 1 = right of the content span; 0.5 = fully committed) over diffusion progress $t/T$ (faint = single rollouts, bold = mean; frozen tail beyond $t/T = 0.6$ cropped). Dashed: the matched left-to-right filler — a causal fill sits on it. Top row: GPQA, MATH, HumanEval (12 rollouts each, default sampler, $C=256$; argmax lock-in as commitment proxy) track the filler. Bottom row: `palindrome_words` fills in bursts, `ends_with` sits above the filler early (the anchored final word commits first), and on `reverse_chain` (depths 4–5) the solving runs (green) *start* at the anchored end and walk backward, while failing runs (grey) rise like a causal fill. $\rho_{\mathrm{chain}}$ = median Spearman of final-commit step vs position; for reverse_chain it is canvas-level (including separators) — digits-only, the correct runs reach $-0.89..-0.97$._
