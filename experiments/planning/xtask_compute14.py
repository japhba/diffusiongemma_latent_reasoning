"""k-doubling battery: new shifts numbers +1/+4/+8/+10/+11/+12, letters +5/+11, and
seed-extension (s2/s3) for the subtraction tasks S3 (-3) and S8 (-8). Spaced-operand
'Numbers:'/'Letters:' format, flat eps0, 4 seeds x 3 fresh subset draws, keys
{tag}|s{s}|ext|n{n}|e{rep}. -> exp/dg_planning/xtask_compute14.json"""
import os
...
F52 = LOW + UPP
VAL = {w: i + 2 for i, w in enumerate(UNITS)}

NQ = ("Pick any number between {lo} and {hi}, write it in words, then write the number "
...
TASKS = {
    "N1":  dict(dom="num", k=1,  lo=2, hi=19, prompt=NQ.format(lo="two", hi="nineteen", rel="one greater")),
    "N4":  dict(dom="num", k=4,  lo=2, hi=16, prompt=NQ.format(lo="two", hi="sixteen", rel="four greater")),
    "N8":  dict(dom="num", k=8,  lo=2, hi=12, prompt=NQ.format(lo="two", hi="twelve", rel="eight greater")),
    "N10": dict(dom="num", k=10, lo=2, hi=10, prompt=NQ.format(lo="two", hi="ten", rel="ten greater")),
    "N11": dict(dom="num", k=11, lo=2, hi=9,  prompt=NQ.format(lo="two", hi="nine", rel="eleven greater")),
    "N12": dict(dom="num", k=12, lo=2, hi=8,  prompt=NQ.format(lo="two", hi="eight", rel="twelve greater")),
    "S3":  dict(dom="num", k=-3, lo=5, hi=20, prompt=NQ.format(lo="five", hi="twenty", rel="three less")),
    "S8":  dict(dom="num", k=-8, lo=10, hi=20, prompt=NQ.format(lo="ten", hi="twenty", rel="eight less")),
    "L5":  dict(dom="let", k=5,  hi="u", prompt=LQ.format(hi="u", off="five")),
    "L11": dict(dom="let", k=11, hi="o", prompt=LQ.format(hi="o", off="eleven")),
}
...
def main():
    out, f = {}, EXP / "xtask_compute14.json"
    if f.exists():
    ...
    for tag, cfg in TASKS.items():
        ...
        eps0 = 0.05 if num else 0.04
        if num:
            pool = [w for w in UNITS if cfg["lo"] <= VAL[w] <= cfg["hi"]]
        else:
    ...
    print("COMPUTE14 DONE", flush=True)
