"""A9: answer resolution over denoising steps — post-hoc (bat_ball, monty) vs load-bearing
(reverse_then_add, sq1000). Solid = mean token entropy over the ANSWER positions, dashed = over
...
PANELS = [("post-hoc", axes[0], [C2[0], C2[9] if len(C2) > 9 else C2[2]]),
          ("true-checking", axes[1], [C2[1], C2[3]])]
for regime, ax, cols in PANELS:
    for c, col in zip([x for x in cases if x["regime"] == regime], cols):
        ...
        if regime == "true-checking":
            ax.annotate(" → ".join(c["answer_path"]), xy=(0.97, 0.97), xycoords="axes fraction",
                        ha="right", va="top", color=col, fontsize="small",
                        xytext=(0.97, 0.97 if c["pid"] == "reverse_then_add" else 0.88))
